...
def load_data(lang, sup_pairs_file='sup_pairs', ref_pairs_file='ref_pairs', additional_pairs=None):
    
    entity1, rel1, triples1 = load_triples(lang + 'triples_1')
    entity2, rel2, triples2 = load_triples(lang + 'triples_2')
    
    
    train_pair = set(load_alignment_pair(lang + sup_pairs_file))
    dev_pair = set(load_alignment_pair(lang + ref_pairs_file))
    
    
    if additional_pairs is not None:
        train_pair.update(additional_pairs)
    
    
    train_pair = np.array(list(train_pair))
    dev_pair = np.array(list(dev_pair))
    ...
    return train_pair, dev_pair, adj_matrix, np.array(r_index), np.array(r_val), adj_features, rel_features
