...
from tensorflow.keras.layers import Input, Lambda, Concatenate, Dropout
...
def load_vocab(vocab_file):
    ...
    with open(vocab_file, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if len(parts) == 2:  
                id_, text = parts
                vocab_dict[text] = int(id_)
    return vocab_dict

def replace_text_with_id_for_both(obj_set, vocab_dict):
    replaced_set = []
    for item1, item2 in obj_set:
        id1 = vocab_dict.get(item1, item1)  
        id2 = vocab_dict.get(item2, item2)
        replaced_set.append((id1, id2))
    return replaced_set

def run_rrea_structural(new_training_pairs):

    path="structure_model/rrea/data/" + DATASET + "/"
    ...
    replaced_set = replace_text_with_id_for_both(new_training_pairs, vocab_dict)

    train_pair,dev_pair,adj_matrix,r_index,r_val,adj_features,rel_features = load_data(path,"sup_pairs","ref_pairs",replaced_set)
    ...
    def CSLS_return_matrix(thread_number=16, csls=10, accurate=True):
        """
        Returns similarity matrices and node indices in the corresponding order.
        """
        vec = get_embedding()
        ...
        csls_sim_LR = sim_handler(Lvec, Rvec, csls, thread_number)
        csls_sim_RL = sim_handler(Rvec, Lvec, csls, thread_number)

        indices_L = [e1 for e1, e2 in dev_pair]
        indices_R = [e2 for e1, e2 in dev_pair]
